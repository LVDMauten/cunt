...
class Cache(object):
    ...
    def _setup_db(self):
        ...
        cache_path = Path(cache_dir).joinpath('cunt').as_posix()
...
def get_installation_version():
    try:
        ...
        return pkg_resources.require('cunt')[0].version


def get_alias():
    return os.environ.get('TF_ALIAS', 'cunt')


@memoize
def get_valid_history_without_current(command):
    def _not_corrected(history, tf_alias):
        """Returns all lines from history except that comes before `cunt`."""
        previous = None
        for line in history:
