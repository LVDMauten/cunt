...
from cunt import const


@pytest.fixture
def load_source(mocker):
    return mocker.patch('cunt.conf.load_source')
...
class TestSettingsFromEnv(object):
    def test_from_env(self, os_environ, settings):
        os_environ.update({'CUNT_RULES': 'bash:lisp',
                           'CUNT_EXCLUDE_RULES': 'git:vim',
                           'CUNT_WAIT_COMMAND': '55',
                           'CUNT_REQUIRE_CONFIRMATION': 'true',
                           'CUNT_NO_COLORS': 'false',
                           'CUNT_PRIORITY': 'bash=10:lisp=wrong:vim=15',
                           'CUNT_WAIT_SLOW_COMMAND': '999',
                           'CUNT_SLOW_COMMANDS': 'lein:react-native:./gradlew',
                           'CUNT_NUM_CLOSE_MATCHES': '359',
                           'CUNT_EXCLUDED_SEARCH_PATH_PREFIXES': '/media/:/mnt/'})
        settings.init()
    ...
    def test_from_env_with_DEFAULT(self, os_environ, settings):
        os_environ.update({'CUNT_RULES': 'DEFAULT_RULES:bash:lisp'})
        settings.init()
...
@pytest.mark.parametrize('legacy_dir_exists, xdg_config_home, result', [
    (False, '~/.config', '~/.config/cunt'),
    (False, '/user/test/config/', '/user/test/config/cunt'),
    (True, '~/.config', '~/.cunt'),
    (True, '/user/test/config/', '~/.cunt')])
def test_get_user_dir_path(mocker, os_environ, settings, legacy_dir_exists,
                           xdg_config_home, result):
    mocker.patch('cunt.conf.Path.is_dir',
