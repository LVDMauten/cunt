import pytest
from cunt.rules.no_command import match, get_new_command
from cunt.types import Command


@pytest.fixture(autouse=True)
def get_all_executables(mocker):
    mocker.patch('cunt.rules.no_command.get_all_executables',
...
def history_without_current(mocker):
    return mocker.patch(
        'cunt.rules.no_command.get_valid_history_without_current',
...
def test_match(mocker, script, output):
    mocker.patch('cunt.rules.no_command.which', return_value=None)
...
def test_not_match(mocker, script, output, which):
    mocker.patch('cunt.rules.no_command.which', return_value=which)
