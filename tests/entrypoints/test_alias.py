...
import pytest
from cunt.entrypoints.alias import _get_alias, print_alias
...
def test_get_alias(monkeypatch, mocker, py2,
    ...
    args = Mock(
        ...
        alias='cunt', )
    mocker.patch('cunt.entrypoints.alias.which', return_value=which)
    shell = Mock(app_alias=lambda _: 'app_alias',
                 instant_mode_alias=lambda _: 'instant_mode_alias')
    monkeypatch.setattr('cunt.entrypoints.alias.shell', shell)
...
def test_print_alias(mocker):
    settings_mock = mocker.patch('cunt.entrypoints.alias.settings')
    _get_alias_mock = mocker.patch('cunt.entrypoints.alias._get_alias')
    known_args = Mock()
