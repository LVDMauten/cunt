...
from cunt.exceptions import EmptyCommand
from cunt.system import Path
...
class TestCorrectedCommand(object):
    ...
    @pytest.mark.parametrize('script, printed, override_settings', [
        ...
        ('git brunch',
         "git brunch || cunt --repeat --force-command 'git brunch'",
         {'repeat': True, 'debug': False}),
        ('git brunch',
         "git brunch || cunt --repeat --debug --force-command 'git brunch'",
...
class TestRule(object):
    def test_from_path_rule_exception(self, mocker):
        load_source = mocker.patch('cunt.types.load_source',
...
class TestCommand(object):
    ...
    def Popen(self, monkeypatch):
        ...
        monkeypatch.setattr('cunt.output_readers.rerun.Popen', Popen)
        return Popen

    @pytest.fixture(autouse=True)
    def prepare(self, monkeypatch):
        monkeypatch.setattr('cunt.output_readers.rerun._wait_output',
                            lambda *_: True)
