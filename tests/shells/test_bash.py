...
class TestBash(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('cunt.shells.bash.Popen')
        return mock

    @pytest.fixture(autouse=True)
    def shell_aliases(self):
        os.environ['TF_SHELL_ALIASES'] = (
            'alias cunt=\'eval $(cunt $(fc -ln -1))\'\n'
            'alias l=\'ls -CF\'\n'
            'alias la=\'ls -A\'\n'
            'alias ll=\'ls -alF\'')
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'cunt': 'eval $(cunt $(fc -ln -1))',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}
    ...
    def test_app_alias(self, shell):
        ...
        assert 'CUNT () {' in shell.app_alias('CUNT')
        assert 'cunt' in shell.app_alias('cunt')
        assert 'PYTHONIOENCODING' in shell.app_alias('cunt')

    def test_app_alias_variables_correctly_set(self, shell):
        alias = shell.app_alias('cunt')
        assert "cunt () {" in alias
        assert 'TF_SHELL=bash' in alias
        assert "TF_ALIAS=cunt" in alias
