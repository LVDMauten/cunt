...
pip3 install cunt --user
```

On FreeBSD, install _Cunt_ with the following commands:

```bash
pkg install cunt
```

On ChromeOS, install _Cunt_ using [chromebrew](https://github.com/skycocker/chromebrew) with the following command:

```bash
crew install cunt
```

On Arch based systems, install _Cunt_ with the following command:

```
sudo pacman -S cunt
```

On other systems, install _Cunt_ by using `pip`:
...
eval $(cunt --alias CUNT)
```

[Or in your shell config (Bash, Zsh, Fish, Powershell, tcsh).](https://github.com/LVDMauten/cunt/wiki/Shell-aliases)
...
**Note: Alias functionality was changed in v1.34 of _Cunt_**

## Uninstall

To remove _Cunt_, reverse the installation process:

- erase or comment _cunt_ alias line from your Bash, Zsh, Fish, Powershell, tcsh, ... shell config
- use your package manager (brew, pip3, pkg, crew, pip) to uninstall the binaries

## How it works

_Cunt_ attempts to match the previous command with a rule. If a match is
...
The following commands are bundled with _Cunt_, but are not enabled by
default:
...
in `~/.config/cunt/rules`. The rule file must contain two functions:
...
`from cunt.conf import settings`

`settings` is a special object assembled from `~/.config/cunt/settings.py`,
...
[More examples of rules](https://github.com/LVDMauten/cunt/tree/master/cunt/rules),
[utility functions for rules](https://github.com/LVDMauten/cunt/tree/master/cunt/utils.py),
[app/os-specific helpers](https://github.com/LVDMauten/cunt/tree/master/cunt/specific/).

##### [Back to Contents](#contents)

## Settings

Several _Cunt_ parameters can be changed in the file `$XDG_CONFIG_HOME/cunt/settings.py`
(`$XDG_CONFIG_HOME` defaults to `~/.config`):

- `rules` &ndash; list of enabled rules, by default `cunt.const.DEFAULT_RULES`;
...
- `CUNT_RULES` &ndash; list of enabled rules, like `DEFAULT_RULES:rm_root` or `sudo:no_command`;
- `CUNT_EXCLUDE_RULES` &ndash; list of disabled rules, like `git_pull:git_push`;
- `CUNT_REQUIRE_CONFIRMATION` &ndash; require confirmation before running new command, `true/false`;
- `CUNT_WAIT_COMMAND` &ndash; the max amount of time in seconds for getting previous command output;
- `CUNT_NO_COLORS` &ndash; disable colored output, `true/false`;
- `CUNT_PRIORITY` &ndash; priority of the rules, like `no_command=9999:apt_get=100`,
  rule with lower `priority` will be matched first;
- `CUNT_DEBUG` &ndash; enables debug output, `true/false`;
- `CUNT_HISTORY_LIMIT` &ndash; how many history commands will be scanned, like `2000`;
- `CUNT_ALTER_HISTORY` &ndash; push fixed command to history `true/false`;
- `CUNT_WAIT_SLOW_COMMAND` &ndash; the max amount of time in seconds for getting previous command output if it in `slow_commands` list;
- `CUNT_SLOW_COMMANDS` &ndash; list of slow commands, like `lein:gradle`;
- `CUNT_NUM_CLOSE_MATCHES` &ndash; the maximum number of close matches to suggest, like `5`.
- `CUNT_EXCLUDED_SEARCH_PATH_PREFIXES` &ndash; path prefixes to ignore when searching for commands, by default `[]`.

For example:

```bash
export CUNT_RULES='sudo:no_command'
export CUNT_EXCLUDE_RULES='git_pull:git_push'
export CUNT_REQUIRE_CONFIRMATION='true'
export CUNT_WAIT_COMMAND=10
export CUNT_NO_COLORS='false'
export CUNT_PRIORITY='no_command=9999:apt_get=100'
export CUNT_HISTORY_LIMIT='2000'
export CUNT_NUM_CLOSE_MATCHES='5'
```
...
to share them with others, create a package named `cunt_contrib_*` with
...
_Cunt_ will find rules located in the `rules` module.

##### [Back to Contents](#contents)

## Experimental instant mode

The default behavior of _Cunt_ requires time to re-run previous commands.
When in instant mode, _Cunt_ saves time by logging output with [script](<https://en.wikipedia.org/wiki/Script_(Unix)>),
then reading the log.

[![gif with instant mode][instant-mode-gif-link]][instant-mode-gif-link]

Currently, instant mode only supports Python 3 with bash or zsh. zsh's autocorrect function also needs to be disabled in order for cunt to work properly.
...
```bash
eval $(cunt --alias --enable-experimental-instant-mode)
```
...
[version-badge]: https://img.shields.io/pypi/v/cunt.svg?label=version
[version-link]: https://pypi.python.org/pypi/cunt/
[workflow-badge]: https://github.com/LVDMauten/cunt/workflows/Tests/badge.svg
[workflow-link]: https://github.com/LVDMauten/cunt/actions?query=workflow%3ATests
[coverage-badge]: https://img.shields.io/coveralls/LVDMauten/cunt.svg
[coverage-link]: https://coveralls.io/github/LVDMauten/cunt
[license-badge]: https://img.shields.io/badge/license-MIT-007EC7.svg
[examples-link]: https://raw.githubusercontent.com/LVDMauten/cunt/master/example.gif
[instant-mode-gif-link]: https://raw.githubusercontent.com/LVDMauten/cunt/master/example_instant_mode.gif
